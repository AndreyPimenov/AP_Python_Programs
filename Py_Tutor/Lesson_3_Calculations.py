...
# alfa = 30*(H + M/60 + S/3600)
alfa = float(input())
# дробная часть часа:  alfa % 30
print( alfa % 30 * 12 ) # минутная в 12 раз быстрее часовой стрелки ПОЯСНЕНИЕ:
# возьмем отрезок времени в 1 час минутная стрелка проходит 60 делений (1час = 60 минут) в то время, как часовая проходит 5 делений
# следовательно 60 : 5 = 12 получим, что минутная стрелка движется в 12 раз быстрее

#15 "Часы 3"

#16 "Проценты"
P, X, Y = int(input()), int(input()), int(input())
k = (X*100 + Y)*((100 + P) / 100) # величина вклада в копейках через год
# специально для 2-го случая: 
if (k%100%1*100000 > 99999):
    k += 1
print (int(k // 100), int(k%100)) # - это правильно 
# Решение разработчиков:
p = int(input())
x = int(input())
y = int(input())
